        ...
        <div class="col-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <form class="forms-sample" method="post" enctype="multipart/form-data" action="../edit_aktivitas_action">
                <input type="hidden" name="id_user" value="<?php if (isset($aktivitas->id_user)) {
                                                              echo $aktivitas->id_user;
                                                            } ?>" required>
                <input type="hidden" name="id" value="<?php if (isset($aktivitas->id)) {
                                                        echo $aktivitas->id;
                                                      } ?>" required>
                <div class="form-group">
                  <label for="exampleInputName1">Waktu</label>
                  <input type="date" class="form-control" name="waktu" value="<?php if (isset($aktivitas->waktu)) {
                                                                                echo $aktivitas->waktu;
                                                                              } ?>" id="exampleInputName1" placeholder="Waktu">
                </div>
                <div class="form-group">
                  <label for="exampleSelectGender">Jenis</label>
                  <select class="form-control" name="jenis" id="exampleSelectGender">
                    <option value="<?php if (isset($aktivitas->jenis)) {
                                      echo $aktivitas->jenis;
                                    } ?>"><?php if (isset($aktivitas->jenis)) {
                                            echo $aktivitas->jenis;
                                          } ?></option>
                ...
                <div class="form-group">
                  ...
                  <input type="file" id="gambar" name="gambar" value="<?php if (isset($aktivitas->gambar)) {
                                                                        echo $aktivitas->gambar;
                                                                      } ?>" class="form-control file-upload-info" required>
                </div>
                <div class="form-group">
                  <label for="exampleTextarea1">Keterangan</label>
                  <textarea class="form-control" name="ket" value="<?php if (isset($aktivitas->ket)) {
                                                                      echo $aktivitas->ket;
                                                                    } ?>" id="exampleTextarea1" placeholder="Tuliskan Keterangan Kegiatan" rows="4"></textarea>
                </div>
                <button type="submit" class="btn btn-primary mr-2">Submit</button>
                <a href="<?php echo site_url('home/aktivitas_hima') ?>" class="btn btn-light">Cancel</a>
              </form>
            </div>
          </div>
        </div>
